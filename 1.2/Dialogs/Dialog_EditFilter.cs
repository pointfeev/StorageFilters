...
internal class Dialog_EditFilter : Window
{
    ...
    public Dialog_EditFilter(ITab_Storage instance, IStoreSettingsParent storeSettingsParent, string key, ExtraThingFilter value,
        ExtraThingFilters tabFilters = null, Dialog_EditFilter previousEditFilterDialog = null) : this(instance, storeSettingsParent)
    {
        this.key = key;
        Filter = value;
    ...
    public Dialog_EditFilter(ITab_Storage instance, IStoreSettingsParent storeSettingsParent, string key, bool keyIsMainFilterString,
        ExtraThingFilters tabFilters) : this(instance, storeSettingsParent, key, null, tabFilters)
    {
        this.keyIsMainFilterString = keyIsMainFilterString;
        Filter = new(storeSettingsParent.GetStoreSettings().filter);
    }

    public override Vector2 InitialSize => new(320f, 230f); //273f);
    ...
    public override void DoWindowContents(Rect winRect)
    {
        ...
        if (tabFilters != null)
        {
            ...
            if (keyIsMainFilterString && mainFilterString is not null)
        ...
        Widgets.Label(new(0f, 0f, winRect.width, editStringY), editString);
        ...
        if (tabFilters != null)
        {
            ...
            curName = Widgets.TextField(new(0f, renameY, winRect.width - 8f - renameStringX, 35f), curName);
            if (Widgets.ButtonText(new(winRect.width - renameStringX, renameY, renameStringX, 35f), renameString))
            {
                _ = CheckCurName();
                Event.current.Use();
            }
            if (!keyIsMainFilterString)
            {
                ...
                Widgets.Label(new(0f, stackLimitY + 35f / 2 - stackSizeStringSize.y / 2, stackSizeStringSize.x, 35f), stackSizeString);
                string stackSizeLimitString = Widgets.TextField(new(stackSizeStringSize.x + 8f, stackLimitY, winRect.width - stackSizeStringSize.x - 8f, 35f),
            ...
            if (Widgets.ButtonText(new(0f, saveLoadY, winRect.width / 2f - 4f, 35f), "ASF_SaveFilter".Translate()))
            {
            ...
            if (Widgets.ButtonText(new(winRect.width / 2f + 4f, saveLoadY, winRect.width / 2f - 4f, 35f), "ASF_LoadSavedFilter".Translate()))
            {
                if (StorageFiltersData.SavedFilters.Count > 0)
                {
                    List<FloatMenuOption> filterFloatMenuOptions = new();
                    ...
                    foreach (KeyValuePair<string, ExtraThingFilter> entry in StorageFiltersData.SavedFilters)
                        filterFloatMenuOptions.Add(new(entry.Key, delegate
                        {
                    ...
                    filterFloatMenu = new(filterFloatMenuOptions);
        ...
        {
            if (Widgets.ButtonText(new(0f, renameY, winRect.width, 35f), "ASF_RemoveThisNIPF".Translate()))
            {
                Find.WindowStack.Add(new Dialog_Confirmation(storageTab, storeSettingsParent, "ASF_ConfirmRemoveNIPF".Translate(key),
                    Filter.NextInPriorityFilter is not null ? "ASF_ConfirmRemoveNIPF_RemoveMore".Translate() : null, delegate
                    {
        ...
        if (!keyIsMainFilterString)
        {
            ...
            if (previousDialog is not null)
            {
                ...
                if (Widgets.ButtonText(new(x, priorityY, backStringX, 35f), backString))
                {
                    StorageFiltersData.SetCurrentFilterDepth(storeSettingsParent, previousDialog.Filter.FilterDepth);
                    Find.WindowStack.Add(previousDialog);
                    Event.current.Use();
            ...
            void EditNIPF()
            {
                ...
                Find.WindowStack.Add(
                    new Dialog_EditFilter(storageTab, storeSettingsParent, nipKey, Filter.NextInPriorityFilter, previousEditFilterDialog: this));
            }
            if (Filter.NextInPriorityFilter is null)
            {
                if (Widgets.ButtonText(new(x, priorityY, width, 35f), "ASF_AddNIPF".Translate()))
                {
                    Filter.NextInPriorityFilter = new(Filter, Filter.FilterDepth + 1);
                    EditNIPF();
                    Event.current.Use();
                }
            }
            ...
            {
                if (Widgets.ButtonText(new(x, priorityY, width, 35f), "ASF_EditNIPF".Translate()))
                {
                    EditNIPF();
                    Event.current.Use();
                }
