...
public static class GenUtils
{
    ...
    public static Rect GetDialogSizeAndPosition(Window dialog, Window editDialog = null)
    {
        ...
        return new Rect(position, new(initialSize.x, initialSize.y)).Rounded();
    }
    ...
    public static IStoreSettingsParent GetSelectedStoreSettingsParent()
        => Find.UIRoot is not UIRoot_Play || Find.MapUI is null ? null : GetStoreSettingsParent(Find.Selector.SingleSelectedObject);
        ...
        string mainFilterString, string tabFilter, Rect position)
    {
        ...
        if (Widgets.ButtonText(position, tabFilter))
        {
            ...
            Dictionary<FloatMenuOption, int> floatMenuOptionOrder = new();
            ...
            filterFloatMenuOptions.Add(NewFilterOption(new(mainFilterString, delegate
            {
                if (Find.WindowStack.WindowOfType<Dialog_EditFilter>() is not null)
            ...
            if (tabFilters.Count > 0)
                foreach (KeyValuePair<string, ExtraThingFilter> entry in tabFilters)
                {
                    ...
                    floatMenuOption = NewFilterOption(new(entry.Key, Action, extraPartWidth: editX + toggleX + removeX, extraPartOnGUI: delegate(Rect extraRect)
                    {
            ...
            filterFloatMenuOptions.Add(NewFilterOption(new("ASF_NewFilter".Translate(), delegate
            {
