...
namespace StorageFilters.Utilities
{
    public static class GenUtils
    {
        ...
        {
            if (MainButtonDefOf.Inspect.TabWindow is MainTabWindow_Inspect inspectPane &&
                StorageFilters.StorageTabRect.HasValue)
            {
        ...
        public static Rect GetDialogSizeAndPosition(Window dialog, Window editDialog = null)
        {
            ...
            Vector2 position = GetDialogPosition((UI.screenWidth - initialSize.x) / 2f,
                                                 (UI.screenHeight - initialSize.y) / 2f, editDialog);
        ...
        public static IStoreSettingsParent GetSelectedStoreSettingsParent()
            => !(Find.UIRoot is UIRoot_Play) || Find.MapUI is null
                ? null
                : GetStoreSettingsParent(Find.Selector.SingleSelectedObject);

        public static void FilterSelectionButton(ITab_Storage instance, IStoreSettingsParent storeSettingsParent,
                                                 ExtraThingFilters tabFilters, string mainFilterString,
                                                 string tabFilter, Rect position)
        {
            ...
            if (Widgets.ButtonText(position, tabFilter))
            {
                ...
                filterFloatMenuOptions.Add(NewFilterOption(new FloatMenuOption(mainFilterString, delegate
                {
                    if (!(Find.WindowStack.WindowOfType<Dialog_EditFilter>() is null))
                        Find.WindowStack.Add(new Dialog_EditFilter(instance, storeSettingsParent, mainFilterString,
                                                                   true, tabFilters));
                ...
                if (tabFilters.Count > 0)
                    foreach (KeyValuePair<string, ExtraThingFilter> entry in tabFilters)
                    {
                        ...
                        {
                            if (!(Find.WindowStack.WindowOfType<Dialog_EditFilter>() is null))
                                Find.WindowStack.Add(new Dialog_EditFilter(instance, storeSettingsParent, entry.Key,
                                                                           entry.Value, tabFilters));
                        ...
                        floatMenuOption = NewFilterOption(new FloatMenuOption(entry.Key, Action,
                                                                              extraPartWidth: editX + toggleX + removeX,
                ...
                FieldInfo optionsFieldInfo = filterFloatMenu.GetType()
                                                            .GetField("options",
                                                                      BindingFlags.NonPublic | BindingFlags.Instance);
                List<FloatMenuOption> options = optionsFieldInfo?.GetValue(filterFloatMenu) as List<FloatMenuOption>;
                optionsFieldInfo?.SetValue(filterFloatMenu,
                                           (from option in options
                                            orderby floatMenuOptionOrder.TryGetValue(option)
